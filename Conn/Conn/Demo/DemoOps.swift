...
enum DemoOps {
    ...
    /// `'/usr/bin/docker' ps -q`。演示响应按 Docker 子命令匹配，因此只在夹具边界
    /// 把已知的演示路径还原成标准命令名，后续各资源分支无需维护两套匹配规则。
    private static func normalizedDockerExecutables(in command: String) -> String {
        command
            .replacingOccurrences(of: "'/usr/bin/docker-compose'", with: "docker-compose")
            .replacingOccurrences(of: "'/usr/bin/docker'", with: "docker")
    }
