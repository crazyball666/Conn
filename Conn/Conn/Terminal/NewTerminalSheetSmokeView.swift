...
struct NewTerminalSheetSmokeView: View {
    ...
    private static let workspace = RemoteWorkspaceSummary(
        workspace: RemoteWorkspaceRef(
            workspaceID: "$demo-session",
    ...
    private static let option = PersistentBackendOption(
        providerID: "tmux",
        displayName: "tmux",
    ...
    private var operations: NewTerminalFlowModel.Operations {
        NewTerminalFlowModel.Operations(
            loadHosts: { [Self.host] },
            persistentBackendOptions: { [Self.option] },
            persistentWorkspaceOptions: { _, _ in [Self.workspace] },
