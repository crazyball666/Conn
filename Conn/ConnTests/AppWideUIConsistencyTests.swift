...
struct AppWideUIConsistencyTests {
    ...
    func paywallUsesFocusedConversionLayout() throws {
        ...
        #expect(source.contains("Text(L(\"将复杂的远程工作，整合至一个工作台\"))"))
        #expect(source.contains("Text(L(\"连接、管理与执行，核心能力集中于一处。\"))"))
        #expect(source.contains("private var legalNotes: some View"))
        #expect(source.contains(".contentMargins(.top, 0, for: .scrollContent)"))
        #expect(source.contains(".padding(.top, ConnSpacing.sm)"))
        #expect(source.contains("PaywallHeroIllustration"))
        #expect(source.contains("LinearGradient("))
        #expect(source.contains("else if context != .upgrade"))
        #expect(source.contains("IconChip(item.systemImage"))
        #expect(source.contains(".lineLimit(2)"))
        #expect(!source.contains("订阅由 Apple 管理，可在系统设置中取消。"))
        #expect(!source.contains("订阅将自动续订，除非在当前计费周期结束前至少 24 小时取消。"))
        #expect(!source.contains("把复杂的主机运维，收进一个工作台"))
        #expect(!source.contains("运行 AI 工具"))
        #expect(!source.contains("专业运维工具箱"))
    ...
    func terminalScreenUsesOptionalInteractionFacet() throws {
        ...
        let host = try packageSource("Sources/ConnTerminal/TerminalHostingView.swift")
        let keybar = try packageSource("Sources/ConnTerminal/TerminalKeybar.swift")
        let composer = try packageSource("Sources/ConnTerminal/TerminalCommandComposer.swift")
        ...
        #expect(composer.contains("Color.connBar"))
        #expect(composer.contains("RadialGradient("))
        #expect(composer.contains(".position(touchLocation)"))
        #expect(composer.contains(".clipped()"))
    ...
    @Test("触摸光效属于整个输入底栏，不在快捷键与输入框之间裁断")
    func terminalTouchGlowBelongsToWholeInputBar() throws {
        let keybar = try packageSource("Sources/ConnTerminal/TerminalKeybar.swift")
        let composer = try packageSource("Sources/ConnTerminal/TerminalCommandComposer.swift")
        let start = try #require(composer.range(of: "struct TerminalInputBar<"))
        let bar = String(composer[start.lowerBound...])
        #expect(!keybar.contains("RadialGradient"))
        #expect(!keybar.contains("Color.connBar"), "An opaque child background would cover the shared glow")
        #expect(bar.contains("TerminalInputBarBackground("))
        #expect(bar.contains("DragGesture(minimumDistance: 0)"))
        #expect(bar.contains(".simultaneousGesture("))
        #expect(bar.contains("touchLocation = value.location"))
        #expect(bar.contains("if isTouchTracking {"))
        #expect(bar.contains("withAnimation(.easeOut(duration: 0.34))"))
        #expect(bar.contains("withAnimation(.easeOut(duration: 0.28))"))
        #expect(bar.contains("if reduceMotion {"))
        #expect(bar.contains(".allowsHitTesting(false)"))
        #expect(bar.contains(".accessibilityHidden(true)"))
    }
