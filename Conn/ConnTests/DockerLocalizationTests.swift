...
extension DockerLocalizationTests {
    ...
    func resourceOperationsUseNavigationMenu() throws {
        ...
        #expect(composeViews.contains("let runtime = viewModel.runtime"))
        #expect(composeViews.contains("runtime: runtime"))
        #expect(!runForm.contains("DockerRunReviewView"))
        #expect(!runForm.contains("showsReview"))
        #expect(runForm.contains("DockerCommand.run("))
        #expect(runForm.contains("runtime: DockerRuntimeContext"))
