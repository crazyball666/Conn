...
public enum ConnThreshold {
    ...
    /// 之所以要留出这段平台期：低负载完全正常，若从 0 就开始往黄端爬，
    /// 用户每天都在看一片发黄的卡片，真正该警觉时反而失去对比。用户反馈
    /// 「60 以上才变黄」起点偏高，遂下调到 40——40 以下恒定绿，
    /// 40→80 渐变到黄，观感上黄色出现得更早、更贴近日常的警觉阈值。
    public static let calm: Double = 40
    /// 超过此值转为警告色。
