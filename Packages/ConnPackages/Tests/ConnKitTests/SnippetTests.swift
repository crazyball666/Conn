...
@Suite("Snippet 变量解析")
struct SnippetTests {
    @Test("用户片段不声明平台且默认没有额外能力和内置 key")
    func userSnippetHasNoPlatformMetadata() {
    ...
    @Test("旧版 JSON 的平台元数据会被安全忽略")
    func legacyPlatformMetadataIsIgnored() throws {
        let json = """
        {
          "id":"legacy", "title":"旧片段", "script":"uptime", "interpreter":"sh",
          "groupIDs":[], "pinned":false, "danger":false, "platforms":["macOS"], "sortOrder":0,
