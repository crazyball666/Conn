...
struct TmuxControlHubTests {
    ...
    @Test("关闭最后一个 Pane 返回 Workspace 已关闭且不再刷新失效拓扑")
    func closingLastPaneReportsWorkspaceClosedWithoutReconciliation() async throws {
        let fixture = try ControlHubFixture()
        let adapter = ScriptedControlHubAdapter()
        let attachmentID = "attachment-close-last-pane"
        let clientID = TmuxClientID(
            targetName: "/dev/pts/28",
            processID: 28,
            createdAt: 28
        )
        let client = TmuxClientSnapshot(
    ...
    @Test("关闭最后一个 Window 返回 Workspace 已关闭且不再刷新失效拓扑")
    func closingLastWindowReportsWorkspaceClosedWithoutReconciliation() async throws {
        let fixture = try ControlHubFixture()
        let adapter = ScriptedControlHubAdapter()
        let attachmentID = "attachment-close-last-window"
        let clientID = TmuxClientID(
            targetName: "/dev/pts/29",
            processID: 29,
            createdAt: 29
        )
        let client = TmuxClientSnapshot(
        ...
        #expect(outcome == .workspaceClosed)
        #expect(await adapter.executedRequests.map(\.operation) == [.killWindow(fixture.window)])
        #expect(await adapter.snapshotRequestCount == 0)
        await hub.releaseLease(observation.lease)
    }
