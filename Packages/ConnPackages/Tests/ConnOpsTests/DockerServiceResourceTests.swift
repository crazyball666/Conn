...
struct DockerServiceResourceTests {
    private let dockerRuntime = DockerRuntimeContext(executable: "docker", sudo: false)
    ...
    func listsVolumes() async throws {
        ...
        """
        let session = try await session([DockerCommand.volumes(runtime: dockerRuntime): json])
        let volumes = try await DockerService.listVolumes(on: session, runtime: dockerRuntime)
        #expect(volumes.map(\.name) == ["pgdata"])
    }

    @Test("dangling 卷名集合")
    func danglingVolumes() async throws {
        let session = try await session([DockerCommand.danglingVolumes(runtime: dockerRuntime): "old_cache\ntmp\n"])
        let names = try await DockerService.danglingVolumeNames(on: session, runtime: dockerRuntime)
    ...
    func listsNetworks() async throws {
        ...
        """
        let session = try await session([DockerCommand.networks(runtime: dockerRuntime): json])
        #expect(try await DockerService.listNetworks(on: session, runtime: dockerRuntime).map(\.name) == ["bridge"])
    }
    ...
    func networkDetail() async throws {
        ...
        """
        let session = try await session([DockerCommand.networkInspect(name: "web", runtime: dockerRuntime): json])
        let detail = try await DockerService.networkDetail(name: "web", on: session, runtime: dockerRuntime)
    ...
    func imageDetailAndHistory() async throws {
        ...
        let session = try await session([
            DockerCommand.imageInspect(reference: "nginx:1.25", runtime: dockerRuntime): inspect,
            DockerCommand.imageHistory(reference: "nginx:1.25", runtime: dockerRuntime): history
        ])
        let detail = try await DockerService.imageDetail(reference: "nginx:1.25", on: session, runtime: dockerRuntime)
        #expect(detail.tags == ["nginx:1.25"])
        let layers = try await DockerService.imageHistory(reference: "nginx:1.25", on: session, runtime: dockerRuntime)
    ...
    func diskUsageDegrades() async throws {
        let session = try await session([DockerCommand.diskUsage(runtime: dockerRuntime): "TYPE  TOTAL  SIZE"])
        #expect(try await DockerService.diskUsage(on: session, runtime: dockerRuntime) == nil)
    }
    ...
    func volumeDetail() async throws {
        ...
        """
        let session = try await session([DockerCommand.volumeInspect(name: "pgdata", runtime: dockerRuntime): json])
        let detail = try await DockerService.volumeDetail(name: "pgdata", on: session, runtime: dockerRuntime)
    ...
    func danglingNetworks() async throws {
        let session = try await session([DockerCommand.danglingNetworks(runtime: dockerRuntime): "old_net\nweb_default\n"])
        let names = try await DockerService.danglingNetworkNames(on: session, runtime: dockerRuntime)
    ...
    func diskUsageParsesRealData() async throws {
        ...
        """
        let session = try await session([DockerCommand.diskUsage(runtime: dockerRuntime): json])
        let usage = try #require(try await DockerService.diskUsage(on: session, runtime: dockerRuntime))
    ...
    func containersUsingVolume() async throws {
        ...
        """
        let session = try await session([
            DockerCommand.containersUsingVolume(name: "pgdata", runtime: dockerRuntime): json
        ])
        let hits = try await DockerService.containersUsingVolume(name: "pgdata", on: session, runtime: dockerRuntime)
        #expect(hits.map(\.name) == ["pg-main"])
    }

    @Test("详情命令失败时保留远端错误")
    func detailCommandFailureIncludesRemoteMessage() async throws {
        let command = DockerCommand.networkInspect(name: "missing", runtime: dockerRuntime)
        let session = try await session([
        ...
        await #expect(throws: DockerQueryError.commandFailed(
            ...
            try await DockerService.networkDetail(name: "missing", on: session, runtime: dockerRuntime)
        }
    }
    ...
    func invalidDetailResponseThrows() async throws {
        let session = try await session([
            DockerCommand.imageInspect(reference: "bad", runtime: dockerRuntime): "not-json"
        ])

        await #expect(throws: DockerQueryError.invalidResponse) {
            try await DockerService.imageDetail(reference: "bad", on: session, runtime: dockerRuntime)
        }
    }

    @Test("资源列表命令非零退出不能伪装成空列表")
    func resourceListFailureThrows() async throws {
        let command = DockerCommand.networks(runtime: dockerRuntime)
        let session = try await session([
        ...
        await #expect(throws: DockerQueryError.commandFailed(
            ...
            try await DockerService.listNetworks(on: session, runtime: dockerRuntime)
        }
    }
    ...
    func containerListFailureThrows() async throws {
        let session = try await session([
            DockerCommand.list(runtime: dockerRuntime): .init(
                stderr: "Cannot connect to the Docker daemon",
                exitCode: 1
            ),
            DockerCommand.stats(runtime: dockerRuntime): .init(stdout: "")
        ])
        ...
        await #expect(throws: DockerQueryError.commandFailed(
            ...
            try await DockerService.list(on: session, runtime: dockerRuntime)
        }
    }

    @Test("详情关联子查询失败不能伪装成没有引用或没有历史")
    func relatedQueryFailuresThrow() async throws {
        let volumeCommand = DockerCommand.containersUsingVolume(name: "data", runtime: dockerRuntime)
        let historyCommand = DockerCommand.imageHistory(reference: "app:1", runtime: dockerRuntime)
        let session = try await session([
