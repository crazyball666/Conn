...
extension SSHClient {
    ...
    enum CommandMode {
        ...
        case ptyExec(request: SSHChannelRequestEvent.PseudoTerminalRequest, command: String)
    ...
    /// Opens a bidirectional SSH exec channel, optionally allocating a PTY before
    /// sending the exec request. Unlike `withPTY`, this never enters a login shell.
    /// The closure remains active for the lifetime of the remote process and owns
    /// the child channel; returning from it closes only that child channel.
    ///
    /// This small public seam is intentionally kept next to Citadel's existing TTY
    /// implementation. It is the transport primitive required by ConnSSH's
    /// `RemoteProcessChannel` abstraction and avoids teaching callers about NIO
    /// child channels or Citadel internals.
    @available(macOS 15.0, *)
    public func withProcess(
        _ command: String,
        terminal: SSHChannelRequestEvent.PseudoTerminalRequest? = nil,
        environment: [SSHChannelRequestEvent.EnvironmentRequest] = [],
        perform: (_ inbound: TTYOutput, _ outbound: TTYStdinWriter) async throws -> Void
    ) async throws {
