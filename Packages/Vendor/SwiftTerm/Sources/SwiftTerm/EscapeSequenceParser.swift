...
public class EscapeSequenceParser {
    ...
    var _osc: cstring
    var _osc52Overflowed: Bool
    public private(set) var osc52OverflowCount: UInt64 = 0
    var _apc: cstring
    var _pars: [Int]
    var _parsTxt: [UInt8]
    ...
    func parse (data: ArraySlice<UInt8>)
    {
        ...
        var dcs = -1
        var osc = self._osc
        var osc52Overflowed = self._osc52Overflowed
        var apc = self._apc
        ...
        while i < end {
                ...
                while j < end {
                    ...
                    if c == ControlCodes.BEL || c == ControlCodes.CAN || c == ControlCodes.ESC {
                        ...
                        if currentState == .apcString {
                            ...
                            if !osc52Overflowed,
                               osc.count >= Terminal.osc52MaximumSequenceBytes,
                               osc.count >= 3,
                               osc[0] == UInt8(ascii: "5"),
                               osc[1] == UInt8(ascii: "2"),
                               osc[2] == UInt8(ascii: ";") {
                                osc52Overflowed = true
                                osc52OverflowCount &+= 1
                            }
                            if !osc52Overflowed {
                                osc.append (c)
                            }
            ...
            case .oscEnd:
                if currentState == .apcString {
                    ...
                    if !osc52Overflowed && osc.count != 0 && code != ControlCodes.CAN && code != ControlCodes.SUB {
                        let oscCode: Int?
        ...
        _osc = osc
        _osc52Overflowed = osc52Overflowed
        _apc = apc
        _collect = collect
        _pars = pars
        _parsTxt = parsTxt
