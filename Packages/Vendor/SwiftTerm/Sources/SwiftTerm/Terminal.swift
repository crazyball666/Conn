 ...
 */
open class Terminal {
    var hostProtocolRevisionStorage: UInt64 = 0
    var hostProtocolSignatureStorage: TerminalHostProtocolSignature?
...
//    }
    ...
    func oscClipboard (_ data: ArraySlice<UInt8>) {
        ...
        if payload.count == 1 && payload[payload.startIndex] == UInt8(ascii: "?") {
            ...
            guard payload.count <= Terminal.osc52MaximumEncodedBytes,
                  payload.count.isMultiple(of: 4) else {
                return
            }
            let padding = payload.suffix(2).reduce(into: 0) { count, byte in
                if byte == UInt8(ascii: "=") { count += 1 }
            }
            let decodedUpperBound = (payload.count / 4) * 3 - padding
            guard decodedUpperBound <= Terminal.osc52MaximumDecodedBytes else {
                return
            }
            let base64 = Data(payload)
            guard let content = Data(base64Encoded: base64),
                  content.count <= Terminal.osc52MaximumDecodedBytes else {
                return
            }
