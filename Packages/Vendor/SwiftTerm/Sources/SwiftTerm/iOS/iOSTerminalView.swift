 ...
 */
public enum SelectionGranularity: Sendable, Equatable {
    case character
    case word
    case row
}
...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    /// When enabled, the embedding application owns direct-touch scrolling,
    /// selection, and mouse gestures. Indirect pointer interactions and typed
    /// host input methods remain available.
    public var hostManagesTouchGestures: Bool = false {
        didSet {
            guard oldValue != hostManagesTouchGestures, didFinishSetup else { return }
    ...
    func enableMousePanGesture () {
        guard !hostManagesTouchGestures, panMouseGesture == nil else {
            return
        }
    ...
    func enableSelectionPanGesture () {
        guard !hostManagesTouchGestures, panSelectionGesture == nil else {
            return
        }
    ...
    func setupGestures ()
    {
        guard !hostManagesTouchGestures, builtInTouchGestures.isEmpty else { return }
        ...
        builtInTouchGestures = [longPress, singleTap, doubleTap, tripleTap]
    }

    private func updateHostManagedTouchGestures() {
        if hostManagesTouchGestures {
            builtInTouchGestures.forEach(removeGestureRecognizer)
            builtInTouchGestures.removeAll()
    ...
    public var hostProtocolState: TerminalHostProtocolState {
        terminal.hostProtocolState
    }

    public var onHostProtocolStateChanged: ((TerminalHostProtocolState) -> Void)? {
        get { terminal.onHostProtocolStateChanged }
        set { terminal.onHostProtocolStateChanged = newValue }
    }

    public func interactionHit(at point: CGPoint) -> TerminalInteractionHit {
        let hit = calculateTapHit(point: point)
        let screen = hit.grid.toScreenCoordinate(from: terminal.displayBuffer)
            ?? Position(
                col: min(max(hit.grid.col, 0), max(terminal.cols - 1, 0)),
                row: min(max(hit.grid.row, 0), max(terminal.rows - 1, 0))
            )
        return TerminalInteractionHit(
            column: screen.col,
            row: screen.row,
    ...
    public func sendHostWheel(
        ...
        terminal.sendHostWheel(direction: direction, count: count, at: hit, modifiers: modifiers)
    }
    ...
    public func makeHostSnapshot(_ scope: TerminalSnapshotScope) -> TerminalBufferSnapshot {
        terminal.makeHostSnapshot(scope)
    }

    public func setHostFocus(_ focused: Bool) {
        terminal.setTerminalFocus(focused)
    }

    public func beginHostSelection(at point: CGPoint, granularity: SelectionGranularity) {
        let position = calculateTapHit(point: point).grid
        switch granularity {
        case .character:
            selection.setSelection(start: position, end: position)
            selection.selectionMode = .character
        case .word:
    ...
    */
    ...
    open func mouseModeChanged(source: Terminal) {
        if source.mouseMode != .off, !hostManagesTouchGestures {
