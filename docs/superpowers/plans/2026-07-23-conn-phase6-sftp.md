...
- [x] 浏览目录：导航/上级/隐藏文件开关；显示大小/权限/时间/类型。演示截图验证。
- [x] 下载（chunked + 进度 + ShareLink）+ 上传（fileImporter + 流式 + 进度）。
- [x] 编辑保存以「打开既有文件+截断写入」保持 inode → 权限/属主不变。**待真机验证**属主。
- [x] 新建文件夹 / 重命名 / 删除（强确认）/ chmod（八进制，保留类型位）。
- [x] 断点续传：`.connpart` + offset 续读，完成原子 rename（会话内 + 重启）。单测覆盖 offset 写。
      **完整传输队列跨重启持久化 + 后台 30s 收尾顺延**（1GB 杀 App 续传部分满足）。
- [x] host 单测覆盖权限串/路径工具/Mock 文件系统 CRUD（ConnSSHTests 50，含 12 SFTP）。
