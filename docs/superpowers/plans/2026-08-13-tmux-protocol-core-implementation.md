...
- [ ] Write failing tests for default, named-socket, and normalized absolute socket-path locators.
- [ ] Reject empty/control/NUL/slash named sockets and relative or parent-traversing socket paths.
- [ ] Add validated `$session`, `@window`, and `%pane` ID wrappers that cannot cross entity kinds.
- [ ] Add a structured, Codable `TmuxServerInstanceToken` containing normalized socket path, PID, and start time.
- [ ] Prove PID reuse/start-time changes and locator changes produce unequal tokens and payloads round-trip losslessly.
...
- [ ] Write failing tests for all first-release Session, Window, and Pane operations.
- [ ] Keep bootstrap creation separate from ordinary token-bound operations.
- [ ] Validate Conn-created names for non-empty bounded UTF-8 and reject C0/C1 controls.
- [ ] Encode every tmux argument with a dedicated tmux command-language encoder.
- [ ] Prove rendered Control commands contain no tmux executable, `-L`/`-S`, or POSIX shell quoting.
...
- [ ] Write failing fixtures for default, `-L`, and `-S` profiles and hostile-but-valid names.
- [ ] Require a validated absolute tmux executable path.
- [ ] Render executable, locator, subcommand, and argv with POSIX argument encoding independent from Control rendering.
- [ ] Add same-invocation server-token guard support; no write operation may use a separate preflight SSH call.
- [ ] Prove the result can be wrapped by a prepared ConnSSH POSIX runtime without a PATH lookup.
...
- [ ] Write failing tests for split DCS/ST markers, byte-at-a-time lines, CRLF, and multiple messages per chunk.
- [ ] Cover tmux 2.6 two-field and newer three-field `%begin/%end/%error` guards.
- [ ] Preserve ordered command output and known/unknown asynchronous notifications.
- [ ] Decode `%output` pane bytes using tmux octal escapes, including non-UTF-8 payload.
- [ ] Reject malformed escapes, unmatched/nested blocks, guard mismatch, line overflow, preamble overflow, and EOF with partial protocol state.
- [ ] Run a deterministic chunk-partition matrix proving every partition yields identical events.

## Completion gate

- [ ] Focused RED is observed before every production type/behavior.
...
- [ ] `ConnMultiplexer` contains no Citadel, NIOSSH, UIKit, SwiftUI, or SwiftTerm imports.
- [ ] Control and Shell renderers share typed operations but no final command string or quoting implementation.
- [ ] No parser buffer is unbounded and unknown notifications never crash parsing.
