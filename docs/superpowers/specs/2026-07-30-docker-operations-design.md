...
操作失败时不清空当前列表或表单；远端 stderr 优先展示，并保留用户的草稿方便修正重试。`RunHistoryEntry` 增加持久化的状态 `.pending` / `.known` / `.unknown`：拉取开始前同步写入 `.pending` 脱敏摘要，拿到最终 `ExecResult` 时用同一记录原子更新为 `.known`，流中断时更新为 `.unknown`。App 启动时将遗留 `.pending` 全部恢复为 `.unknown`，因此被系统终止后再进入仍能如实展示「上次拉取结果未知」。已知退出结果（含非零）记入 `exitCode`；unknown 使用 `nil`，不可显示为成功。Docker 写操作审计只保存脱敏摘要（操作类型、资源名/镜像引用与字段数量），绝不保存原始命令、环境变量值、额外 token、拉取日志或远端输出。

## 交互设计

### 拉取镜像

镜像分段工具栏新增「拉取镜像」。表单仅要求镜像引用；提交后以不可返回的 `fullScreenCover` 进入进度页，滚动展示 Docker 实际流式输出。任务由 `DockerOperationsModel` 持有而非视图 `.task`，分段切换或视图重建不会取消本地观察。活动期间 `fullScreenCover` 禁用手势关闭、不提供导航返回或取消按钮，且 route binding 拒绝被清空；关闭 SwiftUI 页面不能可靠停止远端的 `docker pull`，因此不能制造「已取消」的假象。若 App 被系统终止或流丢失，下一次进入只显示「上次拉取结果未知」，用户可手动刷新镜像列表确认实际状态。成功或已知失败后才允许关闭；成功后刷新镜像、容器关联判定和磁盘占用。
...
- 进程启动前的 pending 审计记录是恢复锚点：App 启动恢复遗留 pending 时不猜测远端状态，只改标为 unknown；已得到最终结果的更新必须与原记录同一 UUID 原子保存。
...
3. Store/App 单测：pending 拉取审计在冷启动后恢复为 unknown、最终结果原子覆盖同一记录；所有 Docker 写操作共享 gate、pull 的已知成功/失败和未知结果、审计脱敏、已知结果的刷新目标、错误时保留草稿，以及强确认的精确输入和 prune 范围改动后重置。
4. 演示数据：补 pull/创建/删除/prune 的确定性 Mock 响应。
5. UI 冒烟：创建容器表单及有效配置复核、拉取日志页（活动时不可返回/关闭，终态后可关闭）、卷和网络的新增/删除确认、prune 范围选择与确认词。
